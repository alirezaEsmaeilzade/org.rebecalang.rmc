...
public class ConvertNetwork {
    ...
    public ConvertNetwork(TimedRebecaCode timedRebecaCode) {
...
//    static public ReactiveClassDeclaration generateReactiveClassDeclaration(TimedRebecaCode timedRebecaCode) {
...
//    }
    ...
    private BinaryExpression getNetworkCondition(ReactiveClassDeclaration sender, ReactiveClassDeclaration receiver, ReactiveClassDeclaration rebec, String senderDefinitionName, String receiverDefinitionName) {
        ...
        leftRightTermPrimary.setName(String.format("_self_%s", rebec.getName().toLowerCase()));
    ...
    private DotPrimary getDotPrimaryStatement(ReactiveClassDeclaration rebec, MsgsrvDeclaration msgsrvDeclaration) {
        ...
        leftDotPrimary.setName(String.format("_self_%s", rebec.getName().toLowerCase()));
    ...
    public void addMsgsrvToNetwork(ReactiveClassDeclaration network, NetworkDeclaration networkDeclaration, ReactiveClassDeclaration rebec) {
        ...
        for (MsgsrvDeclaration msgsrvDeclaration : rebec.getMsgsrvs()) {
            ...
            networkMsgsrv.setName(String.format("_%s_%s", rebec.getName().toLowerCase(), msgsrvDeclaration.getName()));
            ...
            formalParameterDeclaration.setName(String.format("_self_%s", rebec.getName().toLowerCase()));
    ...
    public OrdinaryPrimitiveType getRebecType(String rebecTpyeName) {
    ...
    public OrdinaryPrimitiveType getPrimitiveType(String typeName) {
        for (ReactiveClassDeclaration reactiveClassDeclaration : timedRebecaCode.getReactiveClassDeclaration()) {
            for (FieldDeclaration fieldDeclaration : reactiveClassDeclaration.getKnownRebecs()) {
                for (var types : ((TimedRebecaTypeSystem) fieldDeclaration.getType().getTypeSystem()).getCompatibilityMap().entrySet()) {
                    String checkType = types.getKey().getTypeName();
                    if (types.getKey().getTypeName().equals(typeName)) {
                            return (OrdinaryPrimitiveType) types.getKey();
                        }
                    }
